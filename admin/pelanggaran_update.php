...
$stmt = mysqli_prepare($koneksi, 'UPDATE pelanggaran SET pelanggaran_nama = ?, pelanggaran_point = ? WHERE pelanggaran_id = ?');
if (!$stmt || !mysqli_stmt_bind_param($stmt, 'sii', $nama, $point, $id) || !mysqli_stmt_execute($stmt)) {
    epoin_flash_error('Gagal memperbarui pelanggaran.');
    header('Location: pelanggaran_edit.php?id=' . $id);
    exit;
}
