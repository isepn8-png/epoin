...
/* Full width (preferensi tersimpan) */
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1 class="page-title">
      <span class="title-icon"><i class="fa fa-layer-group"></i></span>
      <span>Kelas</span>
      ...
      <section class="col-lg-12">
        <div class="box">
          ...
          <div class="panel-head">
            ...
            <a href="kelas.php" class="btn btn-soft btn-sm pull-right"><i class="fa fa-reply"></i> &nbsp;Kembali</a>
            <span class="panel-underline"></span>
          </div>

          <div class="box-body">
            <form action="kelas_update.php" method="post" autocomplete="off">
              <?php
              $id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
              $data = mysqli_query($koneksi, "SELECT * FROM kelas WHERE kelas_id=".$id." LIMIT 1");
              ...
              ?>
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" name="nama" required placeholder="Masukkan Nama Kelas .." value="<?php echo htmlspecialchars($d['kelas_nama']); ?>">
                </div>
                ...
                <div class="form-group">
                  ...
                  <select class="form-control" name="jurusan" required>
                    ...
                    while($j = mysqli_fetch_array($jurusan)){ ?>
                      <option value="<?php echo (int)$j['jurusan_id']; ?>" <?php echo ($d['kelas_jurusan']==$j['jurusan_id']?'selected':''); ?>>
                        <?php echo htmlspecialchars($j['jurusan_nama']); ?>
                      </option>
                    <?php } ?>
                  </select>
                  <p class="hint">Pastikan memilih jurusan yang sesuai.</p>
                </div>
                ...
                <div class="form-group">
                  ...
                  <select class="form-control" name="ta" required>
                    ...
                    while($j = mysqli_fetch_array($ta)){ ?>
                      <option value="<?php echo (int)$j['ta_id']; ?>" <?php echo ($d['kelas_ta']==$j['ta_id']?'selected':''); ?>>
                        <?php echo htmlspecialchars($j['ta_nama']); ?> <?php echo ($j['ta_status']=="1"?"(Sekarang)":""); ?>
                      </option>
                    <?php } ?>
                  </select>
                </div>
                ...
                <div class="form-group" style="display:flex; gap:8px; justify-content:flex-end;">
                  <a href="kelas.php" class="btn btn-soft btn-sm"><i class="fa fa-times"></i> Batal</a>
                  <button type="submit" class="btn btn-grad btn-sm"><i class="fa fa-save"></i> Simpan</button>
                </div>
              <?php } else { ?>
                <div class="alert alert-danger">
                  Data kelas tidak ditemukan. <a href="kelas.php" class="alert-link">Kembali ke daftar</a>.
                </div>
              <?php } ?>
            </form>
          </div>
