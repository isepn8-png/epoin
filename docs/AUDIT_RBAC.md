...
**Status:** ✅ **DI-FIX & VERIFIED RUNTIME** (2026-06-27). Tabel `app_meta` (key `rbac_version`); helper `epoin_rbac_version()`/`epoin_rbac_bump_version()`/`epoin_sync_session_perms()` di `auth.php`. Sync 1×/request di `header.php` (reload HANYA bila versi beda). Bump di SEMUA titik mutasi role/permission (save_matrix, save_user_roles, sek_make/revoke, edit_roles, user_update, user_act, assign/revoke_role). Test: UNIT 5/5 + E2E 2-proses PASS — perubahan matrix langsung berlaku ke user lain di request berikutnya **tanpa logout**. Migration: `2026-06-27-001-rbac-version.sql` + APPLY_TO_VPS BLOK 7 (**wajib di-apply di VPS**). Enforcement tetap OFF.
